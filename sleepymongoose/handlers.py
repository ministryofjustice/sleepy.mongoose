...
class MongoHandler:
    ...
    def __init__(self, mongos, replicaSet=None):
        self.connections = {}

        for host in mongos:
            args = MongoFakeFieldStorage({"uri": host, "replicaset": replicaSet})
    ...
    def _get_connection(self, name=None, uri='mongodb://localhost:27017', replicaSet=None):
        if name is None:
            name = "default"
        ...
        try:
            if replicaSet:
                connection = MongoReplicaSetClient(uri, replicaSet=replicaSet, socketTimeoutMS=2000)
            else:
                connection = MongoClient(uri, socketTimeoutMS=2000)
    ...
    def _cmd(self, args, out, name=None, db=None, collection=None, method='GET'):
        if name is None:
            name = "default"

        conn, e = self._get_connection(name)
        if conn is None:
            out('{"ok" : 0, "errmsg" : "couldn\'t get connection to mongo", "exception": "%s"}' % e)
            return
    ...
    def _hello(self, args, out, name=None, db=None, collection=None, method='GET'):
    ...
    def _status(self, args, out, name=None, db=None, collection=None, method='GET'):
    ...
    def _connect(self, args, out, name=None, db=None, collection=None, method='GET'):
        """
        connect to a mongod server or replicaset

        accepts following post arguments:
        uri/server
        replicaset
        name
        """
        ...
        if arg_uri_name:
            try:
                uri = args.getvalue(arg_uri_name)
            except Exception, e:
                print uri
                print e
                out('{"ok" : 0, "errmsg" : "invalid mongodb uri given", "%s" : "%s"}' % (arg_uri_name, uri))
                return
        else:
            uri = 'mongodb://localhost:27017'

        if "replicaset" in args:
            replicaSet = args.getvalue("replicaset")
        else:
            replicaSet = None
        ...
        conn, e = self._get_connection(name, uri, replicaSet=replicaSet)
        if conn is not None:
            out('{"ok" : 1, "server" : "%s", "name" : "%s"}' % (uri, name))
        else:
            out('{"ok" : 0, "errmsg" : "could not connect", "uri" : "%s", "replicaset": "%s", "name" : "%s", "exception": "%s"}' % (uri, replicaSet, name, e))

    def _authenticate(self, args, out, name=None, db=None, collection=None, method='GET'):
        """
    ...
    def _find(self, args, out, name=None, db=None, collection=None, method='GET'):
        """
        query the database.
        """
        ...
        arg_id = MongoHandler._cursor_id
        MongoHandler._cursor_id += 1

        cursors = getattr(self, "cursors")
        cursors[arg_id] = cursor
        setattr(cursor, "id", arg_id)
    ...
    def _more(self, args, out, name=None, db=None, collection=None, method='GET'):
        """
        ...
        arg_id = int(args["id"][0])
        cursors = getattr(self, "cursors")

        if arg_id not in cursors:
            out('{"ok" : 0, "errmsg" : "couldn\'t find the cursor with id %d"}' % arg_id)
            return

        cursor = cursors[arg_id]
    ...
    def _insert(self, args, out, name=None, db=None, collection=None, method='GET'):
        """
        insert a doc
        """
    ...
    def _update(self, args, out, name=None, db=None, collection=None, method='GET'):
        """
        update a doc
        """
    ...
    def _remove(self, args, out, name=None, db=None, collection=None, method='GET'):
        """
        remove docs
        """
    ...
    def _batch(self, args, out, name=None, db=None, collection=None, method='GET'):
        """
